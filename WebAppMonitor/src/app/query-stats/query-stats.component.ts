...
export class QueryStatsComponent implements OnInit {
	...
	constructor(private _statsService: QueryStatsService, public dialog: MdDialog) {
	...
	getColumns() : any[] {
		return [
			...
			{ headerName: "Text", field: "queryText" }
		];
	}
	toggleLoadMask() {
	...
	public changeDate(offset: Number): void {
		this.currentDate = moment(this.currentDate).add(offset, "days").toDate();
		this.loadGridData();
	}
